...
void setup() {
	...
	main.setup();
    main.set_range(0, NUM_LEDS - 1, to_rgb(main.hsv));
...
void loop() {
	...
	if (c) {
		...
		while (c.connected()) {
			while (c.available() > 0) {
				char com[6];
                c.read((uint8_t*)com, 6);
				main.set_variable(com[0], to_hex(com[1]), to_hex4(com + 2));
			}
			delay(10);
		}
		c.stop();
...
struct PatternPerlin : Pattern {
	...
	void tick(Main* main) {
		float t	  = millis();
		float t_h = t / main->m[0];
		float t_s = t / main->m[4];
		float t_v = t / main->m[8];
		...
		for (int i = 0; i < NUM_LEDS; ++i) {
			main->leds[i] = CHSV(
                perlin_value1(tf_h, v_h, i, main->m[3], main->m[1], main->m[2], py0_h, py1_h),
				perlin_value1(tf_s, v_s, i, main->m[7], main->m[5], main->m[6], py0_s, py1_s),
				perlin_value1(tf_v, v_v, i, main->m[11], main->m[9], main->m[10], py0_v, py1_v)
            );
		}
	}
};
...
struct PatternSolidRGB : Pattern {
	void tick(Main* main) {
        main->set_range(0, NUM_LEDS - 1, CRGB(main->hsv.h, main->hsv.s, main->hsv.v));
	}
};
...
struct Pattern2 : Pattern {
	void tick(Main* main) {
        ...
        CRGB col1 = main->hsv;
		CRGB col2 = CHSV(main->hsv.h + main->m[2], main->hsv.s, main->hsv.v);
		CRGB col3 = CHSV(main->hsv.h + main->m[5], main->hsv.s, main->hsv.v);

		uint32_t t		= millis();
        uint16_t phase1 = t * main->m[0];	 // * 64;
        uint16_t phase2 = t * main->m[3];	 // * 3 / 2;
        uint16_t phase3 = t * main->m[6];	 //* 2;

        uint16_t freq1 = main->m[1];	// 0x0300;
        uint16_t freq2 = main->m[3];	// 0x0700;
        uint16_t freq3 = main->m[7];	// 0x0500;

        for (auto &led : main->leds) {
...
struct PatternDither : Pattern {
	void tick(Main* main) {
        CRGB    cols[2] = {CRGB(0, 0, 0), main->hsv};
        uint16_t N = max(main->m[0], (1024 + NUM_LEDS - 1) / NUM_LEDS);
...
struct PatternSparkles : Pattern {
	...
	void tick(Main* main) {
		int		 num_locs  = min(main->m[0], MAX_LOCS);
		uint16_t speed	   = main->m[1];
		uint16_t hue_range = main->m[2];
		...
		uint8_t h = main->hsv.h;
		uint8_t s = main->hsv.s;
		uint8_t v = main->hsv.v;
		...
		for (int i = 0; i < num_locs; i++) {
			...
			if (dt <= -1024 || dt >= 256) {
				...
				main->leds[locs[i].pos] = to_rgb(CHSV(h + locs[i].hue * 4, s, scale8(v, uint8_t((127 - abs(dt - 128)) * 2))));
			}
		}
	}
};
...
struct PatternBlock : Pattern {
	void tick(Main* main) {
		clear(main->leds);
		int		 num  = (main->m[0] >> 3) + 1;
		long	 pos  = (main->m[1] * (long)NUM_LEDS) / (1024 / 256);
		uint16_t ipos = pos >> 8;
        uint8_t	 fpos = pos & 255;
		CRGB	 rgb  = main->hsv;

		main->leds[ipos] = rgb % uint8_t(255 - fpos);
		if (ipos + num < NUM_LEDS) {
            main->set_range(ipos + 1, num - 1, rgb);
        } else {
			main->set_range(ipos + 1, NUM_LEDS - ipos - 1, rgb);
			main->set_range(0, ipos + num - NUM_LEDS, rgb);
		}
...
struct PatternFire : Pattern {
    ...
    void tick(Main* main) {
		...
		for (int i = 0; i < NUM; i++)
			main->leds[i] = Tint(HeatColour(min(heat[i] * 2, 255)), main->hsv);
	}
};
...
struct PatternSweepingDot : Pattern {
	...
	void tick(Main* main) {
		fadeToBlackBy(main->leds, NUM_LEDS, 20);
		...
		if (lastpos > pos)
			main->add_range(pos, lastpos - pos, col);
		else
			main->add_range(lastpos, pos - lastpos, col);
...
struct PatternRainbow : Pattern {
	...
	void	tick(Main* main) {
		rainbow_pos += main->m[0] / 100;
		// FastLED's built-in rainbow generator
		fill_rainbow(main->leds, NUM_LEDS, rainbow_pos, 7);
	}
};
